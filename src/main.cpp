...
void otaStart(const char *linkOTA)
{
  ...
  t_httpUpdate_return ret = ESPhttpUpdate.update(otaWiFi, "http://github.com/Angey60/WiFiMeteoStation/blob/main/firmware.bin");
...
void setup()
{
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    // Запускаем обновление
    otaStart("https://github.com/Angey60/WiFiMeteoStation/blob/main/firmware.bin");
  }
  else
  {
    if (DEBUG)
