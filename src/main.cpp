...
void setClock()
{
  ...
  if (DEBUG) {Serial.print(F("Waiting for NTP time sync: "));}
  int i = 0;
  ...
  while (now < 1000000000)
  {
    ...
    if (i > 60)
    {
      ...
      if (DEBUG) {DEBUG_SERIAL.println();}
      if (DEBUG) {DEBUG_SERIAL.println(F("Time sync failed!"));}
      // return false;
    };
    if (DEBUG) {DEBUG_SERIAL.print(F("."));}
    delay(500);
  }

  // Время успешно синхронизировано, выводим его в монитор порта
  // Только для целей отладки, этот блок можно исключить
  if (DEBUG) {DEBUG_SERIAL.println(F(" ok"));}

  struct tm timeinfo;
  localtime_r(&now, &timeinfo);

  if (DEBUG) {DEBUG_SERIAL.print(F("Current time: "));}
  if (DEBUG) {DEBUG_SERIAL.print(asctime(&timeinfo));}
}
